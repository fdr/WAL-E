        ...
        try:
            with open(et_info.submitted_path, 'rb') as raw_file:
                with StreamPadFileObj(raw_file,
                                      et_info.size) as f:
                    # Only regular files can be padded.
                    assert et_info.filetype == 'REG'
                    assert tarinfo.type == tarfile.REGTYPE

                    # Overwrite target size so the tar header matches
                    # the stream padded stream length.
                    tarinfo.size = et_info.size
                    tar.addfile(tarinfo, f)

                    # Add computed digest so it can be stored with the
                    # backup and used for verification.
